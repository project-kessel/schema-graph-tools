...
//
// It reuses the exact same code paths as the native tools: compiler.CompileGraph
// (via the public compile API + GraphVisitor) produces graph.json, and web.Elements
// applies the one authoritative graph.json -> elements transform. The browser never
...
import (
	...
	"github.com/project-kessel/schema-graph-tools/internal/web"
)
